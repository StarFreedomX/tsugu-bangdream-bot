    ...
    async getFinalCutoffsData(): Promise<MusicRankingBorderResponse | undefined> {
        try {
            return await callAPIAndCacheResponse(
                `${Bestdoriurl}/api/tracker/data?server=${<number>this.server}&event=${this.eventId}&tier=${this.tier}&mid=${this.musicId}`,
                0,
                3
            ) as MusicRankingBorderResponse;
        } catch {
            return undefined;
        }
    }
...
export class MusicRankingCutoffTop {
    ...
    async initFull(_interval = 3600000) {
        ...
        const topData = await callAPIAndCacheResponse(
            `${Bestdoriurl}/api/eventtop/data?server=${<number>this.server}&event=${this.eventId}&mid=${this.musicId}`,
            0,
            3
        ) as MusicRankingTopResponse;
